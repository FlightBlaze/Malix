...
MALIX_NATIVE_FUNCTION(thread_threadNew) {
    ...
    return Value(new boost::thread([&](Value value) {
...
MALIX_NATIVE_FUNCTION(thread_threadSwap) {
    ...
    auto * one = static_cast<boost::thread *>(values[0].getPointer());
    auto * two = static_cast<boost::thread *>(values[1].getPointer());
...
MALIX_NATIVE_FUNCTION(thread_threadJoinable) {
    function->checkArguments(values, 1);
    auto * thread = static_cast<boost::thread *>(values[0].getPointer());
...
MALIX_NATIVE_FUNCTION(thread_threadSleep) {
    function->checkArguments(values, 1);
    boost::this_thread::sleep_for(boost::chrono::duration<double>(values[0].getNumberValue()));
    return Value();
}